...
const groups: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Parking Management", to: "/residents/parking", permission: "residents" }
    ]
  },
  {
  ...
  return (
    ...
    >
            ...
            <div key={group.label}>
              ...
              {!collapsed && open && (
                <div className="mt-1 space-y-1">
                  {group.items
                    .filter((item) =>
                      canView(permissions, item.permission)
                    )
                    .map((item) => (
                      <SidebarLink
      ...
      <div className="space-y-2 border-t border-gray-200 p-3 dark:border-gray-800">
        <div className="flex items-center gap-2 rounded-full border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:border-gray-700 dark:bg-[#1e293b] dark:text-gray-300">
          <Home size={14} className="shrink-0 text-blue-600" />

          {!collapsed && (
            <span className="truncate">
              {society?.society_name ?? "All societies"}
            </span>
          )}
        </div>

        <div className="flex items-center gap-2 rounded-lg bg-blue-50 px-3 py-2 dark:bg-blue-500/20">
          ...
          {!collapsed && onLogout && (
            <button
              type="button"
              onClick={onLogout}
              className="ml-auto shrink-0 rounded-md p-1 text-gray-400 hover:bg-blue-100 hover:text-red-500 dark:hover:bg-gray-700"
            >
              <LogOut size={14} />
            </button>
          )}
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        ...
        ${
          isActive
            ? "border-l-[3px] border-blue-600 bg-blue-50 text-blue-600 dark:bg-blue-500/20 dark:text-blue-300"
            : "border-l-[3px] border-transparent text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-800 dark:hover:text-white"
        }
      `
      }
    >
